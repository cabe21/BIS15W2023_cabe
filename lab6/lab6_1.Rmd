...
msleep %>% 
  ...
  summarize(mean_p = mean(bodywt), min_wt = min(bodywt), max_p = max(bodywt),
  total = n())
...
```{r}
msleep %>% 
  summarise(n_genera=n_distinct(msleep))
```

2. What are the min, max, and mean `sleep_total` for all of the mammals? Be sure to include the total n.
```{r}
msleep %>% 
  summarize(mean_sleep = mean(sleep_total), min_sleep = min(sleep_total), max_sleep = max(sleep_total),
  total = n())
...
```{r}
## na MEANS EMPTY or not there and it is treated the same with calculating means in that you get a NA result 
```

3. Try running the code again, but this time add `na.rm=TRUE`. What is the problem with Cetacea? Compare this to Carnivora. 
```{r}
## there is an na in the cateacea i guess can try to run na in the code 
```
