...
```{r}
ecosphere %>% 
slice_min(order_by = winter_range_area, n = 1, with_ties = TRUE)

```

```{r}
ecosphere %>% 
slice_max(order_by = winter_range_area, n = 1, with_ties = TRUE)
```
...
```{r}
ducks <- filter(ducks, family == "Anatidae")
...
summarise(mean_mass = mean(log10_mass))
```
...
```{r}
eagles %>% 
  mutate(conservation_status = ifelse(population_size < 250000, "threatened", "not_threatned")) 
```

Problem 11. (2 points) Consider the results from questions 9 and 10. Are there any species for which their threatened status needs further study? How do you know?
```{r}
eagles %>% 
  filter(is.na(eagles$population_size))
```
These are the ones that we should look at because they are NA values. 
Problem 12. (4 points) Use the `ecosphere` data to perform one exploratory analysis of your choice. The analysis must have a minimum of three lines and two functions. You must also clearly state the question you are attempting to answer.
```{r}
ecosphere%>% 
  group_by(diet) %>% 
summarise(life_expectancy)
